...
# ============================================================
# Two-tree (forward SPT + reverse SPT) core builder
#
# Replaces the old "monotone / layered violation" corridor DAG with a
# meet-in-the-middle *union of two shortest-path trees*, built on the
# slack-gated candidate subgraph.
#
# Design goals:
#   - maximize distinct o->d options by capturing two SPT frontiers and
#     pruning to nodes on some o->d path
#   - keep it simple and fast for structural diagnostics (cuts/dominators)
#
# Notes:
#   - Dijkstra is run on the gated candidate graph (k nodes) using an O(k^2)
#     implementation (no heap). k is already slack-gated.
#   - We build a single-parent SPT in each direction (true trees).
# ============================================================

@njit(cache=True)
def _dijkstra_dense_spt(
    indptr_loc: np.ndarray,
    indices_loc: np.ndarray,
    pos_loc: np.ndarray,
    w_global: np.ndarray,
    src_loc: int,
):
    """O(k^2) Dijkstra on CSR where weights come from global slot positions."""
    k = indptr_loc.shape[0] - 1
    INF = 1e300
    dist = np.empty(k, np.float64)
    pred = np.empty(k, np.int64)
    ...
    for _ in range(k):
        ...
        a = int(indptr_loc[u])
        b = int(indptr_loc[u + 1])
        for tt in range(a, b):
            v = int(indices_loc[tt])
            p = int(pos_loc[tt])
            w = float(w_global[p])
            nd = best + w
            if nd < dist[v]:
...
def _find_pos_in_cand(
    ...
    """Return global slot position for (u_loc -> v_loc) in candidate CSR, else -1."""
    a = int(cand_indptr[u_loc])
    b = int(cand_indptr[u_loc + 1])
    for tt in range(a, b):
        if int(cand_indices[tt]) == v_loc:
            return int(cand_pos[tt])
    return -1


@njit(cache=True)
def _reach_from_src(indptr_loc: np.ndarray, indices_loc: np.ndarray, src: int):
    k = indptr_loc.shape[0] - 1
...
@njit(cache=True)
def _build_rev_csr(k: int, indptr_loc: np.ndarray, indices_loc: np.ndarray, pos_loc: np.ndarray):
    m = indices_loc.shape[0]
    indeg = np.zeros(k, np.int64)
    for u in range(k):
        a = int(indptr_loc[u])
        b = int(indptr_loc[u + 1])
    ...
    r_indptr = np.zeros(k + 1, np.int64)
    for i in range(k):
        r_indptr[i + 1] = r_indptr[i] + indeg[i]

    r_src = np.empty(m, np.int64)
    r_pos = np.empty(m, np.int64)
    ...
    for u in range(k):
        ...
        for tt in range(a, b):
            ...
            ww = int(cur[v])
            r_src[ww] = u
            r_pos[ww] = int(pos_loc[tt])
            cur[v] = ww + 1

    return r_indptr, r_src, r_pos


@njit(cache=True)
def _build_od_core_csr_numba_two_tree(
    origin_i: int,
    dest_i: int,
    indptr: np.ndarray,
    indices: np.ndarray,
    ...
    """Two-tree meet-in-the-middle corridor core (physical nodes only)."""
    ssp = dist_o[dest_i]
    if not np.isfinite(ssp) or ssp <= 0.0:
        return (np.zeros(1, np.int64),
                np.zeros(0, np.int64),
    ...
    # 2) Candidate CSR (slack-gated)
    outdeg = np.zeros(k, np.int64)
    for i_u in range(k):
        u = int(cand_nodes[i_u])
        du = dist_o[u]
        a = int(indptr[u])
        b = int(indptr[u + 1])
        cnt = 0
        for p in range(a, b):
            ...
            if stamp[v] != cand_stamp:
                continue
            if du + float(w[p]) + dist_to_d[v] > budget:
                continue
            cnt += 1
        outdeg[i_u] = cnt
    ...
    # 3) Reverse CSR for candidate graph
    rev_indptr, rev_src, rev_pos = _build_rev_csr(k, cand_indptr, cand_indices, cand_pos)

    # 4) Dijkstra on gated candidate graph
    dist_fwd, pred_fwd = _dijkstra_dense_spt(cand_indptr, cand_indices, cand_pos, w, o_loc)
    dist_rev, pred_rev = _dijkstra_dense_spt(rev_indptr, rev_src, rev_pos, w, d_loc)

    if not np.isfinite(dist_fwd[d_loc]):
        return (np.zeros(1, np.int64),
                np.zeros(0, np.int64),
                np.zeros(0, np.int64),
                np.zeros(0, np.float64),
    ...
    # 5) Union of the two trees (still k-local)
    outdeg_u = np.zeros(k, np.int64)
    for v in range(k):
        p = int(pred_fwd[v])
        if p >= 0 and v != o_loc:
            outdeg_u[p] += 1
    ...
    uni_indptr = np.zeros(k + 1, np.int64)
    for i in range(k):
        uni_indptr[i + 1] = uni_indptr[i] + outdeg_u[i]

    uni_indices = np.empty(m_u, np.int64)
    uni_pos = np.empty(m_u, np.int64)
    cur = np.empty(k, np.int64)
    for i in range(k):
        cur[i] = uni_indptr[i]
    ...
    for v in range(k):
        ...
        if p >= 0 and v != o_loc:
            t = int(cur[p])
            uni_indices[t] = v
            uni_pos[t] = _find_pos_in_cand(cand_indptr, cand_indices, cand_pos, p, v)
            cur[p] = t + 1
    ...
    for x in range(k):
        ...
        if p >= 0 and x != d_loc:
            t = int(cur[x])
            uni_indices[t] = p
            uni_pos[t] = _find_pos_in_cand(cand_indptr, cand_indices, cand_pos, x, p)
            cur[x] = t + 1

    # 6) Reachability prune (nodes on some o->d path)
    fwd_reach = _reach_from_src(uni_indptr, uni_indices, o_loc)
    uni_rev_indptr, uni_rev_src, _ = _build_rev_csr(k, uni_indptr, uni_indices, uni_pos)
    bwd_reach = _reach_from_src(uni_rev_indptr, uni_rev_src, d_loc)

    core_mask = np.zeros(k, np.uint8)
    core_count = 0
    for i in range(k):
        if fwd_reach[i] and bwd_reach[i]:
            core_mask[i] = 1
            core_count += 1
    ...
    outdeg_c = np.zeros(core_count, np.int64)
    for u in range(k):
        if core_mask[u] == 0:
            continue
        u2 = int(cand2core[u])
        a = int(uni_indptr[u])
        b = int(uni_indptr[u + 1])
        for tt in range(a, b):
            v = int(uni_indices[tt])
            if core_mask[v]:
                outdeg_c[u2] += 1
    ...
    od_indptr = np.zeros(core_count + 1, np.int64)
    for i in range(core_count):
        od_indptr[i + 1] = od_indptr[i] + outdeg_c[i]

    od_indices = np.empty(m3, np.int64)
    od_pos = np.empty(m3, np.int64)
    slot_decay = np.empty(m3, np.float64)

    cur4 = np.empty(core_count, np.int64)
    for i in range(core_count):
        cur4[i] = od_indptr[i]
    ...
    for u in range(k):
        ...
        for tt in range(a, b):
            ...
            t = int(cur4[u2])
            od_indices[t] = v2
            p_global = int(uni_pos[tt])
            od_pos[t] = p_global
            v_gl = int(core_nodes_global[v2])
            slot_decay[t] = float(decay_node[v_gl])
            cur4[u2] = t + 1
...
def main() -> None:
    ...
    for row in tqdm(od_pairs_sorted.itertuples(index=False), total=len(od_pairs_sorted), desc="DAG diagnostics"):
        ...
        od_indptr, od_indices, od_pos, slot_decay, core_nodes, start_loc, end_loc = _build_od_core_csr_numba_two_tree(
            oi, di,
